...
def smiles_to_graph_with_prompt(base_smiles, model, scaler, device, feat_cache, max_length=5, n_virtual_nodes=2):
    ...
    feats = torch.zeros([base_graph.number_of_nodes(), max_node, BID_to_PROMPT_DICT[-1].size()[1]])
    for nid in range(base_graph.number_of_nodes()):
        bid = base_graph.ndata['bid'][nid].item()
        feat = BID_to_PROMPT_DICT[bid]
        feats[nid][:feat.size()[0], :] = feat
...
def smiles_to_graph_without_prompt(smiles, max_length=5, n_virtual_nodes=8, add_self_loop=True):
    d_atom_feats = 138
    d_bond_feats = 14
    # Canonicalize
    mol = Chem.MolFromSmiles(smiles)
