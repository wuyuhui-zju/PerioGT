...
class Evaluator:
    ...
    def _parse_and_check_input(self, y_true, y_pred, valid_ids=None):
        ...
        if not y_true.shape[1] == self.n_tasks:
            raise RuntimeError(
                'Number of tasks for {} should be {} but {} given'.format(self.name, self.n_tasks, y_true.shape[1]))
        ...
        for i in range(y_true.shape[1]):
            ...
            if (self.mean is not None) and (self.std is not None):
                rmse_list.append(np.sqrt(
                    ((y_true[is_labeled, i] - (y_pred[is_labeled, i] * self.std[i] + self.mean[i])) ** 2).mean()))
            else:
