...
class PolyGraphBuilderCopolym:
    ...
    def build(self, smiles_lst, prompt_1, prompt_2):
        co_g = self._build_copolymer_graph(
            smiles_lst,
        ...
        prompt_co = torch.zeros([co_g.number_of_nodes(), self.max_prompt, prompt_1.size()[-1]])
        prompt_co[:prompt_1.size()[0]] = prompt_1
        begin_idx = prompt_1.size()[0] + (self.n_local_nodes - 2)
        end_idx = begin_idx + prompt_2.size()[0]
        prompt_co[begin_idx:end_idx] = prompt_2

        co_g.ndata["prompt"] = prompt_co.half()
        return co_g
    ...
    def smiles_to_prompt(self, base_smiles: str, feat_cache: dict):
    ...
    def _build_graph_with_bid(self, smiles: str, two_mer, query_dict, is_base: bool, n_virtual_nodes=2):
        ...
        for i in range(n_atoms):
            ...
            if len(node_ids) >= 2:
                new_edges = list(permutations(node_ids, 2))
                edges.extend(new_edges)
                new_paths = [[new_edge[0]] + [VIRTUAL_PATH_INDICATOR] * (self.max_length - 2) + [new_edge[1]] for new_edge in
                             new_edges]
                paths.extend(new_paths)
                n_new_edges = len(new_edges)
                line_graph_path_labels.extend([1] * n_new_edges)
