...
def node_emb(smiles, two_mer, query_dict, max_length, n_virtual_nodes, add_self_loop, is_base):
    ...
    if is_base:
        ...
        # hit_at = get_substruct_match_with_timeout(two_mer, modified_patt)
...
def modify(mol):
    ...
    # 对于每个*原子，找到另一个*原子的一个邻居并进行替换
    for i, wildcard_atom in enumerate(wildcard_atoms):
        # 找到另一个*原子
        other_wildcard_atom = wildcard_atoms[1 - i]  # 取索引为0或1的另一个*原子
...
def smiles_to_graph_with_prompt(smiles_lst, model, scaler, device, max_length=5, n_knowledge_nodes=3, n_global_nodes=1):
    co_g = smiles_to_graph_tune(
        smiles_lst,
        max_length=max_length,
        n_knowledge_nodes=n_knowledge_nodes,
        n_global_nodes=n_global_nodes
    )
    feats_1 = smiles_to_prompt(smiles_lst[0], model, scaler, device, max_length=max_length, n_virtual_nodes=2)
    feats_2 = smiles_to_prompt(smiles_lst[1], model, scaler, device, max_length=max_length, n_virtual_nodes=2)
    feats_3 = smiles_to_prompt(smiles_lst[2], model, scaler, device, max_length=max_length, n_virtual_nodes=2)

    feats_co = torch.zeros([co_g.number_of_nodes(), 20, feats_1.size()[-1]])
    feats_co[:feats_1.size()[0]] = feats_1
    start_idx = feats_1.size()[0] + (n_knowledge_nodes - 2)
    end_idx = start_idx + feats_2.size()[0]
    feats_co[start_idx:end_idx] = feats_2

    start_idx = end_idx + (n_knowledge_nodes - 2)
    end_idx = start_idx + feats_3.size()[0]
    feats_co[start_idx:end_idx] = feats_3
    co_g.ndata["prompt"] = feats_co
    return co_g
